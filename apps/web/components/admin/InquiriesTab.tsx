      ...
      >
        {!edit ? (
          <Card variant="muted" className="mb-16">
            ...
            <div className="rb-label mt-16 mb-8">AI解析</div>
            <p className="rb-small" style={{ margin: 0 }}>
              {inquiry.ai_description || "—"}
            </p>
            <div className="rb-tiny muted-text mt-8">タグ: {inquiry.tags.join(" / ") || "—"}</div>
            <div className="rb-tiny muted-text mt-8">
              確定物品:{" "}
              {cands.find((match) => match.status === "confirmed")?.item?.display_id ||
